...
def shop_trip() -> None:
    ...
    for index_for_car, customer in enumerate(list_customers):
        ...
        for shop in list_shop:
            ...
            sum_total = sum(this_shop_price.values()) + this_shop_price_drive
            print(f"{customer.name}\'s trip to the {shop.name} "
                  f"costs {sum_total}")
            full_price.append(sum_total)
        cheaper_store = list_shop[full_price.index(min(full_price))]
        if customer.money - min(full_price) >= 0:
